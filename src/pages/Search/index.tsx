    ...
    useEffect(() => {
        ...
        const getData = async () => {
            Promise.all(requests).then(res=>{
                setPageState({
                    songs:resolveSongs((res[0] as SearchSongRes).result.songs, 'search'),
                    albums:(res[1] as SearchAlbumRes).result.albums.map(item => {
                                const { id, name, artists } = item;
                                const picUrl = rp(item.picUrl);
                                const singers = artists.map(({ id, name }) => ({ id, name }));
                                return { id, name, picUrl, artists:singers };
                            }),
                    artists:(res[2] as SearchSingerRes).result.artists.map(item => {
                                            const { id, name } = item;
                                            const picUrl = rp(item.picUrl);
                                            return { id, name, picUrl };
                                        }),
                    playlists:(res[4] as SearchPlaylistRes).result.playlists.map(item => {
                                const { id, name, description } = item;
                                const picUrl = rp(item.coverImgUrl);
                                return { id, name, coverImgUrl:picUrl, description };
                            }),
                    videos:(res[3] as SearchVideosRes).result.videos!.map((item)=>{
